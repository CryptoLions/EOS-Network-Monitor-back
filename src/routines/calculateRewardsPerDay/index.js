...
const getInflation = async () => {
  const systemData = await api.getCurrencyStats({ symbol: 'XEC', code: 'eosio.token' });
  if (!systemData.XEC || !systemData.XEC.supply){
      return 0;
  }
  const totalSupply = castToInt(systemData.XEC.supply.split(' ')[0]);
